function app() {
  return {
    ...
    // Load all quotes (IndexedDB first, API fallback)
    async loadQuotes() {
      ...
      if (db && db.db) {
        try {
          ...
          if (cachedQuotes.length > 0) {
            ...
            console.log('Loaded', cachedQuotes.length, 'quotes from IndexedDB');
          }
        } catch (cacheError) {
          console.warn('Failed to load quotes from IndexedDB:', cacheError);
        }
      }
      ...
      try {
        ...
        if (db && db.db) {
          try {
            ...
            console.log('Saved', this.quotes.length, 'quotes to IndexedDB');
          } catch (saveError) {
            console.warn('Failed to save quotes to IndexedDB:', saveError);
          }
        }
      } catch (error) {
        if (!loadedFromCache) {
          ...
          console.log('API failed, using cached quotes');
        }
      }
    },
    ...
    // Load all data (IndexedDB first, API fallback)
    async loadData() {
      try {
        const db = window.offlineDB;
        let loadedFromCache = false;
        ...
        if (db && db.db) {
          try {
            const [cachedItems, cachedMaterials, cachedClients] = await Promise.all([
              db.getWorkItems().catch(() => []),
              db.getMaterials().catch(() => []),
              db.getClients().catch(() => [])
            ]);
            
            if (cachedItems.length > 0) {
              this.workItems = cachedItems;
              loadedFromCache = true;
            ...
            if (cachedMaterials.length > 0) {
              ...
              loadedFromCache = true;
              console.log('Loaded', cachedMaterials.length, 'materials from IndexedDB');
            }
            ...
            console.warn('Failed to load from IndexedDB:', cacheError);
          }
        }
        
        // Always try to fetch from API (for fresh data)
        try {
          ...
          console.log('Loaded fresh data from API:', this.workItems.length, 'items');
        } catch (apiError) {
          if (!loadedFromCache) {
            console.error('Failed to load from API and no cache:', apiError);
            showToast('📴 Delam offline - uporabljam lokalne podatke', 'warning');
          } else {
            console.log('API failed, using cached data');
            showToast('📴 Brez povezave - uporabljam shranjene podatke', 'info');
          }
        }
        ...
        console.log('Data ready:', this.workItems.length, 'items,', this.materials.length, 'materials');
    ...
    async triggerSync() {
      ...
      if (!db || !db.db) {
        if (window.showToast) {
          window.showToast('⚠️ Offline način ni na voljo', 'warning');
        }
        return;
      }
    ...
    async updatePendingCount() {
      ...
      try {
        const [pendingQuotes, pendingClients] = await Promise.all([
          db.getPendingQuotes().catch(() => []),
          db.getByIndex('clients', 'syncStatus', 'pending').catch(() => [])
        ]);
        
        this.pendingCount = pendingQuotes.length + pendingClients.length;
      } catch (error) {
